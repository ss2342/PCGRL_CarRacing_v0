...
class Model(Module):
    ...
    def __init__(self, reward, hidden_size):
        super().__init__()
        self.hidden_size = hidden_size
        self.embedding = torch.nn.Embedding(reward, hidden_size)
        self.gru = torch.nn.GRU(reward, hidden_size, batch_first=True, bidirectional=False)

        self.seq1 = torch.nn.Sequential( 
                        torch.nn.Linear(hidden_size, hidden_size/2, activation='LeakyReLu'),
                        torch.nn.Linear(hidden_size/2, 18, activation="LogSoftMax"))

        self.seq2 = torch.nn.Sequential( 
                        torch.nn.Linear(hidden_size, hidden_size/2, activation='LeakyReLu'),
                        torch.nn.Linear(hidden_size/2, 4, activation="LogSoftMax"))
    ...
    def forward(self, input, hidden):
        ...
        output1 = self.seq1(torch.flatten(output[:, -1, :]))
        output2 = self.seq2(torch.flatten(output[:, -1, :]))
        output3 = self.seq3(torch.flatten(output[:, -1, :]))
        return [output1, output2, output3], hidden
...
def test_model():
    ...
    model = Model(1000, 256)
    optim = torch.optim.Adam(model.parameters(), lr=0.004, amsgrad=True)
    encoder_hidden = encoder.initHidden()
    output, encoder_hidden = model(
                        [checkpoints rewardsum], encoder_hidden)
    print(model)
